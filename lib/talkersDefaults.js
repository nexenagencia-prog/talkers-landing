...
export async function ensureTalkersV9(sb){
  if(!sb) return;
  ...
  if(Number(settings?.theme_json?.ui_version||0)>=9) return;
  ...
  const mergedTheme={...(settings?.theme_json||{}),ui_version:9};
  ...
  else await sb.from('site_settings').insert({...TALKERS_SETTINGS,theme_json:{...TALKERS_SETTINGS.theme_json,ui_version:9},updated_at:new Date().toISOString()});
}
