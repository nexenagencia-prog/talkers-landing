...
export async function ensureTalkersV11(sb){
  if(!sb) return;
  ...
  if(Number(settings?.theme_json?.ui_version||0)>=11) return;
  ...
  // V11: replace only the existing FAQ content once. Future CMS edits are preserved
  // because ui_version 11 prevents this migration from running again.
  const faqHome=(existingSections||[]).find(x=>x.slug==='recursos' && x.kind==='faq' && x.enabled!==false);
  const faqContent=TALKERS_SECTIONS.find(x=>x.slug==='recursos' && x.kind==='faq')?.content;
  if(faqHome?.id && faqContent) await sb.from('sections').update({content:faqContent,updated_at:new Date().toISOString()}).eq('id',faqHome.id);

  const mergedTheme={...(settings?.theme_json||{}),ui_version:11};
  ...
  else await sb.from('site_settings').insert({...TALKERS_SETTINGS,theme_json:{...TALKERS_SETTINGS.theme_json,ui_version:11},updated_at:new Date().toISOString()});
}
