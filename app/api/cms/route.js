...
export async function GET(){if(!(await guard()))return NextResponse.json({error:'unauthorized'},{status:401});const sb=supabaseAdmin();if(!sb){const c=getSupabaseConfigStatus();return NextResponse.json({error:c.reason||'Supabase não configurado'},{status:503});}const [{data:settings,error:e1},{data:nav,error:e2},{data:sections,error:e3}] = await Promise.all([sb.from('site_settings').select('*').limit(1).maybeSingle(),sb.from('nav_items').select('*').order('sort_order'),sb.from('sections').select('*').order('sort_order')]);if(e1||e2||e3)return NextResponse.json({error:(e1||e2||e3).message},{status:500});return NextResponse.json({settings,nav,sections});}
