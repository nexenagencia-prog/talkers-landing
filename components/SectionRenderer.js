...
export default function SectionRenderer({section,settings}){
  ...
  if(kind === 'connection') return <section id={section.slug} className={`${cls} connection-section`} style={style}><div className="section-inner"><div className="connection-panel"><div className="connection-main"><div className="connection-copy"><div className="connection-kicker">{c.eyebrow}<span>{c.badge||'01'}</span></div><h2>{c.title}</h2><p>{c.text}</p></div><div className="plane-art" aria-hidden="true"><div className="plane-body"/><div className="plane-wing"/><div className="plane-tail"/></div></div><div className="connection-bottom"><div className="connection-stats">{(c.items||[]).map((x,i)=><div className="connection-stat" key={i}><span>{x.label}</span><strong>{x.value}</strong></div>)}</div><div className="connection-brief"><h3>{c.brief_title}</h3><p>{c.brief_text}</p></div></div></div></div></section>;
